 ...
 * 							 1.데이터를 보호할 목적
 * 							 2. 경계선
 * 							 3. 메소드와 변수를 한개로 묶어서 외부에서 직접 적은하지 못하게 만든다
 * 								=> getter/setter
 * 									 읽기    쓰기   => lombok
 * 							 4. 접근 지정어 사용법
 * 							 5. 웹에서 주로 캡슐화 사용
 ...
 *				단일 상속만 가능
 *				class A
 *				class B extends A => A클래스를 상속받은 B
 *				    A
 *					↑  ==> 상속기호	 
 *					B
 *			=> 다형성 => 유지보수와 관련
 ...
 *					  호출되는 위츠는 반드시 생성자에서 호출
 *					  생성자의 첫줄에서만 사용이 가능
 *			super() => 상속 내린 클래스의 생성자 호출
 *						호출하는 위츠는 반드시 생성자에서 호출
 *						생성자의 첫줄에서 사용이 가능
 *			
 */		
class Student
{
	...
	public Student(int b, String name) {
		stnum=b;
		this.name=name;
	}	
}
...
public class 객체지향용어_1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("std.stnum="+std.stnum);
		System.out.println("std.name="+std.name);
		Student std1=new Student(2);
		System.out.println("std1.stnum="+std1.stnum);
		System.out.println("std1.name="+std1.name);
		Student std2=new Student("심청이");
		System.out.println("std2.stnum="+std2.stnum);
		System.out.println("std2.name="+std2.name);
		Student std3=new Student(3,"이순신");
		System.out.println("std3.stnum="+std3.stnum);
