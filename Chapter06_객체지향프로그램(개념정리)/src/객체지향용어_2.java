 ...
 * 			  | 인스턴스 변수 (데이터 보호)
 * 		   private
 * 			 : 같은 클래스 내부에서만 접근이 가능 => 자신 클래스에서만 사용이 가능
 * 		     : 인스턴스 변수 => 은닉화
 * 		   default : 같은 폴더까지만 접근이 가능
 * 		   protected : 같은 폴더까지만 접근이 가능 , 다른 폴더인 경우에 상곡이 있어야 접근 가능
 ...
 * 			{ 
 * 				public abstract void display(); // 선언만 하는 메소드
 * 			}
 * 		3. 패키지 제작
 * 			
 * 		[접근지정어] 리턴형 메소드명(매개변수...)
 * 		{
 * 			return 값 ==> 생략 void => 생략이 되면 자동으로 컴파일러가 첨부
 * 		}	  |
 * 			메소드 종료
